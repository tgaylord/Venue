...
- **Email deliverability (live but fragile):** booking/status emails are **best-effort** (a send failure never blocks the booking, so a broken send is silent). `EMAIL_FROM` is `noreply@venuedash.com` but **that Resend domain is still unverified** — sends will fail until you verify the domain's DNS (or point `EMAIL_FROM` at a verified / `resend.dev` address). Verify before real owner outreach; **if testing shows no email arriving, this is the cause, not a code bug.**
