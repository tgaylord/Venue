...
| Decision | Choice |
|---|---|
| Waitlist storage | **Resend contacts** via server action — no DB schema in this phase (tables arrive in Domain core). (The SDK v6 deprecated Audiences; contacts are account-level now, which serves the same purpose.) |
...
- Server action `joinWaitlist`:
  ...
  3. Call `addWaitlistContact(email)` (`lib/waitlist.ts`) → Resend SDK `contacts.create` (account-level; Audiences are deprecated in SDK v6, so no audience ID is needed).
...
- No new env vars or console steps — the existing `RESEND_API_KEY` covers contact creation; signups appear under the Resend account's Contacts.
...
- **Unit tests** (Vitest, mocked Resend): valid/invalid email handling; `addWaitlistContact` calls the Contacts API with the right email; duplicate maps to success; honeypot short-circuits without an API call.
- **CI** unchanged (lint / typecheck / test / build) and must stay green; landing + legal pages compile as static routes.
- **Manual verification:** side-by-side visual check against the prototype landing; a real submit on the Vercel preview lands a contact in Resend's Contacts; `/terms` and `/privacy` render; `/dashboard` auth gating unaffected.
...
- Submitting the waitlist form adds a contact in Resend; duplicates and failures behave as specified.
